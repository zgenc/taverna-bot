...
bot.on('text', async (ctx) => {
  ...
  // Bekle uyarısı ve spam kontrolü tamamen kaldırıldı
...
bot.command('ozet', async (ctx) => {
  try {
    ...
    const summaryPrompt = `Son 150 mesajın kapsamlı bir özetini çıkar.
En az 30 mesajın içeriğini kapsayacak şekilde detaylı yaz.
Ana konuları, önemli mesajları, tartışmaları ve dikkat çeken kısımları mutlaka belirt.
Kısa tutma, bilgilendirici ve akıcı olsun.

${sohbet}`;

    const completion = await openai.chat.completions.create({
      messages: [
        { role: "system", content: "Sen kapsamlı ve detaylı özet çıkaran bir asistansın. Kısa kesme." },
...
- Özet: /ozet (son 150 mesajın detaylı özeti, en az 30 mesaj kapsar).
